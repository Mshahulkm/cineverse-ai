...
function posterStyle(m){
  ...
  return `background:linear-gradient(135deg,hsl(${h} 70% 35%),hsl(${h2} 75% 25%))`;
}
...
function initHeroPosters(){
  ...
  const picks=[...state.movies].sort((a,b)=>(b.rating||0)-(a.rating||0)).slice(0,40);
  ...
  positions.forEach(([x,y],i)=>{
    ...
    d.setAttribute('style',`${posterStyle(m)};left:${x}px;top:${y}px;animation-delay:${i*-1.5}s`);
...
function cardHTML(m){
  ...
  return `<article class="card reveal" data-id="${m.id}">
    ...
    <div class="poster" style="${posterStyle(m)}"><div class="title">${escape(m.title)}</div></div>
    <div class="meta"><span>${m.year||'—'}</span><span class="rating">★ ${rating}</span></div>
  </article>`;
}

function renderTrending(){
  const row=$('#trendingRow');
  const top=[...state.movies].filter(m=>m.rating).sort((a,b)=>b.rating-a.rating).slice(0,18);
  row.innerHTML=top.map(cardHTML).join('');
}

function renderCollections(){
  const collections=[
    {name:'Top Rated', sub:'Highest IMDb scores', filter:m=>m.rating>=7.5},
    {name:'Hidden Gems', sub:'Underrated picks', filter:m=>m.rating&&m.rating>=6.5&&m.rating<7.5},
    {name:'Action & Adventure', sub:'High octane', filter:m=>m.genres.some(g=>/action|adventure/i.test(g))},
    {name:'Mind Bending', sub:'Sci‑fi & mystery', filter:m=>m.genres.some(g=>/sci|mystery|thriller/i.test(g))},
    {name:'Feel Good', sub:'Comedy & family', filter:m=>m.genres.some(g=>/comedy|family|romance/i.test(g))},
    {name:'Dark & Twisted', sub:'Horror & crime', filter:m=>m.genres.some(g=>/horror|crime/i.test(g))},
    {name:'Animation', sub:'For all ages', filter:m=>m.genres.some(g=>/animation|anime/i.test(g))},
    {name:'Drama Hall', sub:'Acclaimed dramas', filter:m=>m.genres.some(g=>/drama|biograph|histor/i.test(g))},
  ];
  $('#collections').innerHTML=collections.map((c,i)=>{
    ...
    return `<div class="col-card reveal" data-col="${i}">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:14px"><b>${count} movies</b><span>→</span></div>
    </div>`;
  }).join('');
